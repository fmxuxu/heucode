...
def build_candidates(args, nodes, data_file, save_cand_file, tokenizer):
    ...
    jnodes = tuple([tuple(n["tokens"]) for n in nodes])#(('Canberra'),('Xinhua','News',……))

    jnodes_bpe = []
    for n in tqdm(nodes):
        bpes = []
        for w in n["tokens"]:#'Xinhua'
            # tmp = tokenizer.tokenize(w)
            bpes += tokenizer.tokenize(w)#['Xi','##nh','##ua']
        jnodes_bpe.append(tuple(bpes))
    jnodes_bpe = tuple(jnodes_bpe)#node元組(('Canberra'),('Xinhua', 'News', 'Agency'),……)

    jdatas = tuple([tuple(d["tokens"]) for d in data])#句子元组
    ...
    jdatas_bpe = tuple(jdatas_bpe)#句子中Bpe元组

    entities = tuple([
        tuple([
            tuple(e["tokens"])
        for e in d["entities"]])
    for d in data])#按句子组成每个实体的元组
    ...
    entities_bpe = tuple(entities_bpe)#按句子组成每个实体bpe的元组

    node_ids = tuple([n["id"] for n in nodes])
    node_types = tuple([n["type"] for n in nodes])
    # 参数args.num_process指定了进程池中进程的数量，表示要启动的进程数，会根据机器的CPU核数等情况进行调整。
...
if __name__ == "__main__":
    ...
    pretrained_model = {
        ...
        "genia": "dmis-lab/biobert-base-cased-v1.2",
    }
