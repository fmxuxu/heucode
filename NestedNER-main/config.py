...
class PreEmbeddedLM(nn.Module):
    ...
    def forward(self, batch_tokens, maxlen=None):
        ...
        # embs = np.stack(embs, axis=0) # (B, T, H)#1*768
...
class Config:
    ...
    def __init__(self, args):
        ...
        self.data_name = args.data_dir.split("/")[-1]

        self.tokenizer = BertTokenizer.from_pretrained(
            args.pretrained_model, do_lower_case=args.do_lower_case
        )

        self.entity_type2idx = utils.build_entity_type2idx("/mnt/LJH/XFM/NERProject/spanCL/data/datasets/genia/entity_types.json", {"None": 0})
    ...
    def load_word_embed(self, file_path, word_dim, special_tokens):
        ...
        for idx, word in enumerate(idx2word[len(special_tokens):]):
            word_embeds[idx] = np.asarray(glove[word], dtype=float)
    ...
    def get_num_update_epoch(self, data_size):
        ...
        self.update_freq = self.update_freq#表示多少个batch更新一次模型

        self.t_steps = (self.data_size // (self.batch_size * self.update_freq) +
                        int(self.data_size % (self.batch_size * self.update_freq) != 0)) * self.max_epoch#总共需要迭代的次数
