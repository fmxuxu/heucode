...
#用于设置一些随机数生成器的种子，以便在每次运行程序时都能产生相同的伪随机数。具体来说，在设置种子之后，每次程序中涉及到随机性的模块或函数（如 random、numpy.random、torch）所生成的随机数序列都是相同的。
...
def get_grad_norm(model):
    ...
    total_norm = total_norm ** (1. / 2)#4.999998997829366 5.000000030417906
    return total_norm
...
def node_nearest_cands(nodes, edges, n_order_neighbor):
    ...
    """
    # print("进入node_nearest_cands")
    G = nx.Graph()
    G.add_nodes_from([n["id"] for n in nodes])
    G.add_edges_from([(e["node1"]["id"], e["node2"]["id"], {"weight": e["weight"]}) for e in edges])
    # print(G.graph)
    A = nx.to_numpy_matrix(G, weight="weight")#用G构建邻接矩阵A
    nodes_topK_cands = partition_arg_topK(-A, n_order_neighbor, axis=1).tolist()#找到每个node相似的前3个node,[10317,3]，没有相似的就随机整了三个node
    ...
    # import pdb;pdb.set_trace()
    return entities_cands#没有相似的话就是[]，[10317,m],m可能是空，可能是一个数，也可能是2个数，还可能是3个数
