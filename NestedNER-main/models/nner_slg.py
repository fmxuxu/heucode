...
class Char_Encoder(nn.Module):
    ...
    def forward(self, char_ids):
        ...
        x = self.char_embeds(char_ids)#2*122*68 embedding（54，30）==》2*122*68*30
        bsz, w_maxlen, c_maxlen, _ = x.shape
        x = x.view(bsz * w_maxlen, c_maxlen, -1)#244*68*30
        lens = lens.view(bsz * w_maxlen)#244，

        word_seq_lens = lens + (lens == 0).long() #244 # avoid length == 0
        word_rep = x#244*68*30
        sorted_seq_len, permIdx = word_seq_lens.sort(0, descending=True)
        _, recover_idx = permIdx.sort(0, descending=False)
        sorted_seq_tensor = word_rep[permIdx]#244*68*30

        packed_words = pack_padded_sequence(sorted_seq_tensor, sorted_seq_len.cpu(), True)# 413*30 batch_sizes=tensor([244,  33,  31,  26,  22,  18,  14,   9,   8,   6,   1,   1]
        lstm_out, (h, _) = self.lstm(packed_words, None)#h 2*244*50 _ 2*244*50 lstm_out 413*100 batch_sizes=tensor([244,  33,  31,  26,  22,  18,  14,   9,   8,   6,   1,   1])
        # lstm_out, _ = pad_packed_sequence(lstm_out, batch_first=True)
        # outputs = lstm_out[recover_idx]
        hidden = torch.cat([h[-2, :, :], h[-1, :, :]], dim=-1)# 244*100
        hidden = hidden[recover_idx]
        out = hidden.view(bsz, w_maxlen, -1)#2*122*100
...
class NNER_SLG(Base_Model):
    ...
    def _encode_to_span_level(self, h_word_level, entity_masks):
        ...
        bsz, n_span, seq_len = entity_masks.size()# bsz 2 n_span 108 seq_len 122
        ...
        if bsz * n_span >= max_n_span:
            ...
            m = (entity_masks.unsqueeze(-1) == 0).float() * (-1e30)# 2*108*122*1  # B n_span L 1
            h_span_level = m + h_word_level.unsqueeze(1).repeat(1, n_span, 1, 1) # B n_span L H
            h_span_level = h_span_level.max(dim=2)[0] # 2*108*400  # B n_span H
    ...
    def _forward_gcn(
        ...
        for i in range(batch_size): # i-th batch
            ...
            for j, (b, e, batch_g) in enumerate(batch_sgs[i]): # j-th node
                ...
                features = torch.cat(nodes_feature, dim=0) # 1*425 n_node_in_be, gcn_in_dim
    ...
    def _fine_tune_lm_embed(
        ...
        """
        #lm_bpe_ids lm_attn_masks 2*137
        lm_out = self.lm(
    ...
    def _encode_to_word_level(
        ...
        if self.use_char_encoder or self.use_word_encoder:
            ...
            h_wc = word_lstm_forward(
                lstm=self.wc_lstm_enc, x=h_wc, mask=(word_ids != 0)
            ) # 2*122*400 40*58*400 B, max_n_word, wc_lstm_h
        else:
            if self.use_lm_embed:
                if not is_entity:
                    # h_lm, lm_mask, h_cls = self.lm_embed(word_strs, maxlen=512)
    ...
    def _get_logits(self, sample):
        ...
        ) # 得到每句话中的每个word的表示（char+word+plm）2*122*400 2*768 # B * max_n_word * h_lm (or 2 * wc_lstm_h), B * h_lm

        h_span_level = self._encode_to_span_level(
            h_word_level=h_word_level, entity_masks=sample["entity_masks"]
        ) # 2*108*400 B * max_n_span * (2 * wc_lstm_h)#得到每个span的maxpooling表示1*36*400

        if self.use_size:#得到可学习的宽度的表示1*36*25
            h_span_size = self.size_embeds(sample["entity_sizes"]) #2*108*25 B max_n_span h_size
        #！！！！！！！！！！！！！！！！！！！！！！！！得到的原始的span表示
        if self.use_gcn:
            # add h_ctx.unsqueeze(1).repeat(1, h_pool.shape[1], 1) or not
            # B * max_n_span * (h_span_level+h_size) # +H_ctx
            if self.use_size:
                span_reprs = torch.cat([h_span_level, h_span_size], dim=2)# 2*108*425
            else:
            ...
            for i in range(bsz):
                ...
                _h_word_level, _ = self._encode_to_word_level(
                    ...
                    is_entity=True,
                )  # 40*58*400(n_span * n_node) * max_n_word * (2 * wc_lstm_h), None

                # _h_span_level = _h_word_level.max(dim=1)[0]
                _h_span_level = _h_word_level.mean(dim=1) # _h_span_level 1*400 40*400_h_word_level1*58*400(n_span*n_node) * (2*wc_lstm_h)
                if self.use_size:
                    h_size = self.size_embeds(
                        torch.LongTensor(flat2(sample["sgs_nodes_size"][i], False)).to(
                            self.dev)
                    )  # 1*25 40*25 (n_span * n_node) * h_size

                    # (n_span * n_node) * (2 * wc_lstm_h + h_size)
                    _h_span_level = torch.cat([_h_span_level, h_size], dim=1)#1*425 40*425
                ...
                idx = 0
                _cur_span_node_reprs = []
                n_span = len(sample["sgs_nodes_char_ids"][i])#36 108
                for i_span in range(n_span):
                    n_node = len(sample["sgs_nodes_char_ids"][i][i_span])#1
                    _cur_span_node_reprs.append(_h_span_level[idx: idx+n_node, :])
                    idx += n_node
                node_reprs.append(_cur_span_node_reprs)#[[36][108]]



            h_gcn_node, h_gcn_graph = self._forward_gcn(
                span_reprs=span_reprs,  # 2*108*425 B n_span h_span_level+h_size
                node_reprs=node_reprs,  #[[36] [108]] B n_span n_node h_span_level+h_size
                batch_sgs=sample["sgs"],  # [[graph36][graph108]]List[List[Graph]]
                batch_sgs_nodes=sample["sgs_nodes"],  #[[36][208]] List[List[List[Int]]]
                n_node_for_batch_and_span=n_node_for_batch_and_span,  # [[36][108]]List[List[Int]]
        ...
        # B n_span h_gcn_span+h_gcn_graph+h_size 2*108*(384+425)

        h_span = self.span_dropout(h_span)#2*108*809
        logits_span = self.fc_out_span(h_span) # 2*108*8 B n_span label_size

        # node representations
        logits_node = None
        if self.use_gcn:
            logits_node = self.fc_out_node(h_gcn_node) # n_node_in_B label_size 41*384-->41*8

        return logits_span, logits_node

    def _get_span_loss(self, logits_span, entity_types, entity_sample_masks):
        #logits_span2*108*8 entity_types entity_sample_masks 2*108
        logits_span = logits_span.view(-1, logits_span.shape[-1])#216*8
        labels_span = entity_types.view(-1)#216
        ...
        )#tensor(319.7176, device='cuda:0', grad_fn=<SumBackward0>)

        if loss_span > 1e6:  # Weibo NER dataset has a huge loss in one instance (id=143)
            loss_span *= 0

        sample_size_span = int(entity_sample_masks.sum())# int 144
        if sample_size_span > 0:
            loss_span = loss_span / sample_size_span# tensor(2.2203, device='cuda:0', grad_fn=<DivBackward0>)
        else:
            loss_span = 0 * loss_span

        return loss_span, sample_size_span

    def _get_node_loss(self, logits_node, sgs_node_types):#logits_node41*8 sgs_node_types[[36][108]]
        labels_node = []
        for i in range(len(sgs_node_types)):
            for j in range(len(sgs_node_types[i])):
                labels_node.extend(sgs_node_types[i][j])
        labels_node = torch.LongTensor(labels_node).to(self.dev)#41,
        sample_mask = torch.ones(labels_node.size()).to(self.dev)#41,
        loss_node = Cross_Entropy(
            logits_node, labels_node, sample_masks=sample_mask,
            is_logits=True, pad_idx=int(1e8)  # 1e8 means no pad
        )#tensor(83.4108, device='cuda:0', grad_fn=<SumBackward0>)
        if loss_node > 1e6:  # Weibo NER dataset has a huge loss in one instance (id=143)
            loss_node *= 0

        sample_size_node = logits_node.shape[0]#41
        if sample_size_node > 0:
            loss_node = loss_node / sample_size_node#tensor(2.0344, device='cuda:0', grad_fn=<DivBackward0>)
        else:
    ...
    def forward(self, sample):
        ...
        # loss of spans
        logits_span, logits_node = self._get_logits(sample)#logits_span 2*108*8 logits_node 41*8
        ...
        )#tensor(2.2203, device='cuda:0', grad_fn=<DivBackward0>) int144
        ...
        loss = loss_span + self.alpha * loss_node#tensor(2.4237, device='cuda:0', grad_fn=<AddBackward0>)

        log_output = {
            "loss": loss.data.item(),#'loss': 2.4237020015716553
            "loss_span": loss_span.data.item(),#'loss_span': 2.2202608585357666
            "loss_node": loss_node.data.item(),#'loss_node': 2.0344107151031494
            "sample_size_span": sample_size_span,#'sample_size_span': 144
            "sample_size_node": sample_size_node,#'sample_size_node': 41
        }
    ...
    def aggregate_log_outputs(self, log_outputs):
        ...
        loss = sum(log.get('loss', 0) for log in log_outputs) #9.672465324401855
        loss_span = sum(log.get('loss_span', 0) for log in log_outputs) #8.856061935424805
        loss_node = sum(log.get('loss_node', 0) for log in log_outputs) #8.164034605026245
        sample_size_span = sum(log.get('sample_size_span', 0) for log in log_outputs) # 144[36,108]+205+214+112=675
        sample_size_node = sum(log.get('sample_size_node', 0) for log in log_outputs) #128
        ...
        #{'loss': 9.672465324401855, 'loss_span': 8.856061935424805, 'loss_node': 8.164034605026245, 'sample_size_span': 675, 'sample_size_node': 128, 'p': 0, 'r': 0, 'f': 0, 'extra_eval_data': None}
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            choices=[
                ...
                "bert-base-chinese", "dmis-lab/biobert-base-cased-v1.2"
            ],
        ...
        parser.add_argument('--max_span_size', default=8,type=int)
        ...
        parser.add_argument('--gcn_norm_method',  default="right",type=str)
