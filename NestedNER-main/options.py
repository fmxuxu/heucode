...
def add_dataset_arguments(parser):
    ...
    group.add_argument('--data_dir',
                       type=str,
                       default='/mnt/LJH/XFM/NERProject/spanCL/data/datasets/genia',
    ...
    group.add_argument('--train_ratio', type=float, default=1, help='train data ratio')
    group.add_argument('--valid_ratio', type=float, default=1, help='valid data ratio')
    group.add_argument('--test_ratio', type=float, default=1, help='test data ratio')

    group.add_argument('--max_n_word', type=int, default=150, help='maximum number of words in sentences')
    group.add_argument('--max_ent_n_word', type=int, default=80, help='maximum number of words in entities')
    group.add_argument('--max_word_len', type=int, default=80, help='maximum number of words in entities')
...
def add_model_arguments(parser):
    ...
    group.add_argument('--use_size', type=int, default=0)

    group.add_argument('--char_embed_dim', default=30, type=int)
    group.add_argument('--char_hid_dim', default=50, type=int)
    group.add_argument('--word_embed_dim', default=200, type=int) # the same as wv_file
    group.add_argument('--cls_embed_dim', default=768, type=int) # the same as LM
    group.add_argument('--wc_lstm_hid_dim', default=300, type=int) # the same as LM
    group.add_argument('--sent_enc_dropout', default=0.2, type=float) # the same as LM
    group.add_argument('--lm_dim',
                       default=768, type=int,
                       help="768/1024 for base/large model, 4096 for flair embeddings")

    group.add_argument('--wv_file', default="/mnt/LJH/XFM/NERProject/spanCL/data/datasets/genia/GENIA.pubmed.200d.txt", type=str)
    group.add_argument('--lm_emb_path', default="/mnt/LJH/XFM/NERProject/spanCL/data/datasets/genia/GENIA.biobert_base_cased_flair.emb.pkl",  type=str)
    group.add_argument('--ent_lm_emb_path',
                       default="/mnt/LJH/XFM/NERProject/spanCL/data/datasets/genia/GENIA.ent_biobert_base_cased_flair.emb.pkl",
                       type=str)
    ...
    group.add_argument('--use_gcn', type=int, default=0)
    ...
    group.add_argument('--data_name',default="genia", type=str)


    # graph
    group.add_argument('--edge_weight_threshold', default=0.8, type=float,
                        help='keep edges whose weight higher than the threshold')
    group.add_argument('--graph_neighbors', default="5 3", type=str,
                       help='number of graph neighbors for each hop from span')
    group.add_argument('--max_batch_nodes', default=3000, type=int)
    group.add_argument('--max_batch_edges', default=40000, type=int)

    group.add_argument('--concat_span_hid', type=int, default=1)
    ...
    # group.add_argument('--gcn_dropout', default=0.1, type=float)
    # group.add_argument('--gcn_norm_method', default="right", type=str, help="optional: mean, attn")
...
def add_optimization_arguments(parser):
    ...
    group.add_argument('--other_lr', default=3e-4, type=float)
    # group.add_argument('--weight_decay', default=1e-10, type=float)
    group.add_argument('--clip_norm', default=10, type=float,
    ...
    group.add_argument('--max_epoch', default=30, type=int)
...
def add_checkpoint_arguments(parser):
    ...
    group.add_argument('--save_dir', default='/mnt/LJH/XFM/NERProject/spanCL/__models/genia/nner_slg', type=str,
...
def post_process_arguments(args):
    # args.gcn_layers = "384 384"
    args.gcn_layers = "625 384"
...
def get_train_arguments():
    ...
    parser.add_argument('--save_log_file', type=str,default="/mnt/LJH/XFM/NERProject/spanCL__results/genia/nner_slg/trnn_41_18.pkl", help='file path to save log')
    parser.add_argument('--save_best_result_file', type=str,default="/mnt/LJH/XFM/NERProject/spanCL/__results/genia/nner_slg/best_results.txt", help='file path to the best result')

    # evaluation during training
    parser.add_argument('--evaluate_per_update', default=-1, type=int,
                        help="-1 is no evaluation within a epoch")

    # eval low-frequency entities
    parser.add_argument('--do_extra_eval', type=int, default=1,
                        help="do extra evaluation, including saving "
                             "the predictions, subgraphs, training time, inference time, etc")
    parser.add_argument('--save_pred_dir', type=str,default="/mnt/LJH/XFM/NERProject/spanCL/__results/genia/nner_slg/41_18")

    parser.add_argument('--LOGFILENAME', type=str,default="/mnt/LJH/XFM/NERProject/spanCL/__results/genia/nner_slg/41_18.log")
