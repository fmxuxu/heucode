...
def pad_char_ids(raw_char_ids, pad_id=0, max_n_char=150, max_n_word=512):
    char_ids = []
    for char_ids_i in raw_char_ids[:max_n_word]:  # 限制单词数量不超过 max_n_word
        n_pad = max_n_char - len(char_ids_i)
        if n_pad > 0:
            char_ids.append(char_ids_i + ([pad_id] * n_pad))
        else:
            char_ids.append(char_ids_i[:max_n_char])  # 如果单词的字符数量超过 max_n_char，直接截断
...
def pad_word_ids(raw_word_ids, pad_id=0, max_n_word=512):
    word_ids = raw_word_ids[:max_n_word]  # 截断到 max_n_word
    n_pad = max_n_word - len(word_ids)
    if n_pad > 0:
...
def collate_fn_padding(batch):
    ...
    for key in keys:
        ...
        if key in [
            ...
            "sgs_nodes_size",
        ]:#直接将原样本中的列表作为新字典中对应key的值，并不进行 padding 处理
            new_batch[key] = samples
        elif hasattr(batch[0][key], "shape"):
            if not batch[0][key].shape:
                new_batch[key] = torch.stack(samples)#以第 0 维度进行 stack 操作
            else:
