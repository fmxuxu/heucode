...
class NER_Dataset(torch.utils.data.Dataset):
    ...
    # 将每个字符（包括特殊符号）使用事先定义好的映射表char2idx替换成唯一的id和未知id，并统计未知字符的数量。
    ...
    def __getitem__(self, index):
        ...
        span_cands = None
        if self._use_gcn:
            # print("self._span_cands:",self._span_cands,"\nindex:",index)
    ...
    # 把每个字符与一个整数序号一一对应起来，并返回从字符到数字的映射表。
