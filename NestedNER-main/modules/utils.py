...
def Cross_Entropy(outputs, labels, sample_masks, is_logits=True, pad_idx=None):
    ...
    """
    if is_logits:
        log_probs = F.log_softmax(outputs, dim=-1)#outputs 216*8-->log_probs216*8
    else:
        log_probs = torch.log(outputs + 1e-13)
    log_probs = log_probs.view(-1, log_probs.size(-1))#216*8

    nll_loss = F.nll_loss(
        log_probs, labels.view(-1),
        ignore_index=pad_idx, reduction='none'
    )#216*8-->216
    nll_loss = (nll_loss * sample_masks).sum()#一个数
    # nll_loss = F.nll_loss(
    #     log_probs, labels.view(-1),
